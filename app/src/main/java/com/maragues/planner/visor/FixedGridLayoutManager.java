...
/**
 * Extracted from https://github.com/devunwired/recyclerview-playground
 */
  ...
  public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    if (state.isPreLayout()) {
      ...
      if (removedCache.size() == 0 && mChangedPositionCount > 0) {
        for (int i = mFirstChangedPosition; i < (mFirstChangedPosition + mChangedPositionCount);
            i++) {
    ...
    if (getChildCount() == 0) { //First or empty layout
       ...
       */
      if (!state.isPreLayout() && getVerticalSpace() > (getTotalRowCount()
